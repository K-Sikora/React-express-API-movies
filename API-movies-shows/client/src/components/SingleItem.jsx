...
const SingleItem = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/${itemType}/${id}`).
      then(response => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/${itemType}/keywords/${id}`)
      .then(response => {
        console.log(response.data)
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/${itemType}/similar/${id}`)
      .then(response => {
      console.log(response.data)
    ...
    <div className='bg-stone-800 text-white'>
      ...
      {itemData && 
        <div className='min-h-screen gap-6 bg-stone-800 flex flex-col   py-5 px-0 justify-start'>
            <ItemInfo episodesVisible={episodesVisible } setEpisodesVisible={setEpisodesVisible} itemData={itemData} />
            <ItemCoverImage itemData={itemData} />
            <ItemDetails itemData={itemData} keyword={keyword} setIsVisibleKeywords={setIsVisibleKeywords} isVisibleKeywords={isVisibleKeywords} />

            <Keywords isVisibleKeywords={isVisibleKeywords} keyword={keyword} itemData={itemData} />
            <Similar similar={similar} itemType={itemType} />
        </div> 
        }
