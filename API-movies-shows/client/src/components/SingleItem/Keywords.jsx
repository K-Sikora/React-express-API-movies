...
const Keywords = (props) => {
  return (
    ...
    <AnimatePresence>
    {
        props.isVisibleKeywords && <motion.div
          animate={{ opacity: 1 }}
          initial={{opacity:0}}
          exit={{opacity: 0}}
          className='flex gap-4 flex-wrap  px-5'>
        {props.keyword.keywords && props.keyword.keywords.map((k, index) => (
          <motion.button
            animate={{ opacity: 1 }}
            initial={{opacity:0}}
        ...
        {props.keyword.results && props.keyword.results.map((k, index) => (
          <motion.button
            animate={{ opacity: 1 }}
            initial={{opacity:0}}
            transition={{delay: index/20}}
      ...
      <div className='flex gap-2 flex-wrap px-5'>
            {props.itemData.genres && props.itemData.genres.map((item, index) => (
