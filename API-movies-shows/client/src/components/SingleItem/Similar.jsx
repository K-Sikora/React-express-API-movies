...
const Similar = (props) => {
  return (
    ...
    <>
  {props.similar && <div className='px-5 flex flex-col gap-6 '> 

<h3 className='text-2xl text-stone-100'>
  {props.itemType === 'movie' ? 'See similar movies' : 'See similar shows'}
  </h3>
    <div className='flex scrollbar overflow-x-auto overflow-y-hidden gap-4 h-96 '>

    {props.similar.results && props.similar.results
      .slice(0, 14)
      .map((similarItem, index) => (
        similarItem.backdrop_path && 
        <div key={index} className='flex flex-shrink-0 relative  w-40 items-end '>

          <div className='absolute top-0 left-0 '>
            <Link
                to={`/${props.itemType}/${similarItem.id}`}
            >
            <div className='relative h-full w-full  '>
          <img className='rounded-md w-full h-full object-cover ' src={`https://image.tmdb.org/t/p/original` + similarItem.poster_path} alt='background photo' />
