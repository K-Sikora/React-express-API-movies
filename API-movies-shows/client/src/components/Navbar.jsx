...
import { faBars, faSearch, faMoon, faClose, faStar } from '@fortawesome/free-solid-svg-icons'
...
const Navbar = () => {
    ...
    const { data: searchResults, isLoading, isError } = useQuery(['searchResults', currentQuery], async() => {
        const response = await axios.get(`http://localhost:8080/api/search-movie?query=${currentQuery}`)
    ...
    return (
      ...
      <motion.div variants={parentVariant} animate="visible" initial="hidden" className=' py-4 md:py-5 w-full relative px-5 flex items-center max-w-6xl mx-auto'>
          <AnimatePresence>
          {searchVisible && (
                  <motion.div
                      animate={{ opacity: 1 }}
                ...
                <div className='relative w-full h-full'>
                    <input onBlur={mobileFocusLeaveHandler} onFocusCapture={updateSearchHandler} onChange={updateSearchHandler} autoFocus className=' w-full h-full px-4 bg-stone-900 outline-none focus:outline-2 focus:-outline-offset-4 text-white focus:outline-emerald-400 pr-12' placeholder='Search for a movie/show...' />
                                    <FontAwesomeIcon className='absolute cursor-pointer right top-0 text-2xl right-0 p-5 text-white' onClick={() => {
          ...
          <motion.div variants={childVariant} className='w-1/2 md:w-1/4 md:mr-4 lg:mr-0 text-xl '>

            <Link to='/' className='flex justify-start items-center gap-6 text-emerald-50 '>
              <img src="../public/logo.png" className='w-8' alt="logo" />
              <h2 className='text-base text-emerald-50 first-letter:text-emerald-400 -ml-4  tracking-wide font-semibold pointer-events-none'>findastream</h2>
            </Link>
          </motion.div>
            ...
            {searchResults && isFocused && searchResults
                ...
                .map((item, index) => {
                    return <Link key={index} to={`/${item.media_type}/${item.id}`}> <motion.div
                        ...
                        animate={{ opacity: 1 }} initial={{ opacity: 0 }} transition={{delay: index/20}}
                        ...
                        <div className='flex flex-col justify-around'>
                            ...
                            {item.vote_average !== 0 && <span className='flex items-center gap-1'>
                                
                                { item.vote_average && <FontAwesomeIcon className=' text-emerald-500 text-lg' icon={faStar}></FontAwesomeIcon>}
                                
                                <h4 className=''>{
                                    item.vote_average ? item.vote_average.toFixed(2) : ''
                                }</h4>
                            </span>}
                            
                            {
                                item.media_type == 'movie' ? <h5 className='text-sm'>Movie{item.release_date && ', ' + item.release_date.slice(0,4)}</h5> :
                ...
                <motion.div variants={childVariant} className='w-1/2 md:w-1/4 flex  items-center justify-end gap-6  text-xl text-emerald-50'>

                    <FontAwesomeIcon className='cursor-pointer md:hidden' onClick={() => { setSearchVisible(!searchVisible) }} icon={faSearch}></FontAwesomeIcon>
                    <button className='flex items-center justify-center gap-2'>
                    <FontAwesomeIcon className='text-lg' icon={faBars}></FontAwesomeIcon>
